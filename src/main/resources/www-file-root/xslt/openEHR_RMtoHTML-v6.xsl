...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" 
	...
	xmlns:fn="http://www.w3.org/2005/xpath-functions" xmlns:res="http://www.imt.liu.se/mi/ehr/2010/xml-result-v1#">
	...
	<xsl:template match="res:xml-results">
	...
	<xsl:template match="res:binding"> <!-- FIXME: TODO: make it also work for xsi:type='PrefixX:...  and for types without any prefix... -->
		<xsl:choose>
			<xsl:when test="./*[starts-with(@xsi:type, 'v1:ITEM_')]">
				<xsl:call-template name="generic-ITEM_STRUCTURE">
					<xsl:with-param name="itemsnode" select="*" />
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="./*[starts-with(@xsi:type, 'v1:CLUSTER')]">
				<xsl:call-template name="generic-CLUSTER">
					<xsl:with-param name="clusternode" select="*" />
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="./*[starts-with(@xsi:type, 'v1:ELEMENT')]">
	...
	<xsl:template match="v1:timing">
		<span class="header">Timing:</span>
		<xsl:choose>
			<xsl:when test="v1:formalism/text() = 'HL7GTS'">
			HL7GTS(<xsl:value-of select="v1:value/v1:value/@xsi:type" />, 
			<xsl:value-of select="v1:value/v1:value/@code" />)
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="v1:value" />
				(
				<xsl:value-of select="v1:formalism" />
				)
			</xsl:otherwise>
		</xsl:choose>
	...
	<!-- .............. Start helper templates ................. These are for 
		element names commonly in XML instances with consistent semantic meaning 
		or simply special-purpose templates for common operations on particular types 
		of nodes. -->
	<!-- Generic name Uses generic-dv-text -->
	...
	<!-- Generic date (in format is '2006-05-05T10:44:46'; out format is '05/05/2006') -->
			...
			<xsl:when test="not(substring($datetimestring, 5,1) = '-')">
				<xsl:if test="not(substring($datetimestring, 5,2) = '')">
					<xsl:if test="not(substring($datetimestring, 7,2) = '')">
						<xsl:value-of select="substring($datetimestring, 7,2)" />
						/
					</xsl:if>
					<xsl:value-of select="substring($datetimestring, 5,2)" />
					/
				</xsl:if>
	...
	<!-- Generic time (in format is '2006-05-05T10:44:46'; out format is '10:44 
		AM +9:30') -->
	<xsl:template name="generic-time">
		...
		<xsl:choose>
			...
			<xsl:when test="not(substring($datetimestring, 5,1) = '-')">
				<xsl:choose>
					<xsl:when test="substring($datetimestring, 9,1) = 'T'">
						<xsl:if test="not($display-as-24-hour)">
							<xsl:choose>
								<xsl:when test="substring($datetimestring, 10,2) &lt; '01'">
									12
								</xsl:when>
								<xsl:when test="substring($datetimestring, 10,2) &gt; '12'">
									<xsl:value-of select="number(substring($datetimestring, 10,2))-12" />
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="substring($datetimestring, 10,2)" />
								</xsl:otherwise>
							</xsl:choose>
						</xsl:if>
						<xsl:if test="substring($datetimestring, 12,2)">
							:
							<xsl:value-of select="substring($datetimestring, 12,2)" />
							<xsl:if
								test="substring($datetimestring, 14,2) and $show-seconds='yes'">
								:
								<xsl:value-of select="substring($datetimestring, 14,2)" />
							</xsl:if>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<xsl:choose>
							<xsl:when test="not($display-as-24-hour)">
								<xsl:choose>
									<xsl:when test="substring($datetimestring, 9,2) = '00'">
										12
									</xsl:when>
									<xsl:when
										test="substring($datetimestring, 9,2) &gt; '12' or substring($datetimestring, 9,2) &lt; '01'">
										<xsl:value-of select="number(substring($datetimestring, 9,2))-12" />
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="substring($datetimestring, 9,2)" />
									</xsl:otherwise>
						...
						<xsl:if test="substring($datetimestring, 11,2)">
							:
							<xsl:value-of select="substring($datetimestring, 11,2)" />
							<xsl:if
								test="substring($datetimestring, 13,2) and $show-seconds='yes'">
								:
								<xsl:value-of select="substring($datetimestring, 13,2)" />
							</xsl:if>
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:if test="not($display-as-24-hour)">
					<xsl:choose>
						<xsl:when test="substring($datetimestring, 10,2) &gt;= '12'">&nbsp;PM
						</xsl:when>
						<xsl:otherwise>&nbsp;AM
						</xsl:otherwise>
					</xsl:choose>
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="not($display-as-24-hour)">
						<xsl:choose>
							<xsl:when test="substring($datetimestring, 12,2) = '00'">
								12
							</xsl:when>
							<xsl:when test="substring($datetimestring, 12,2) &gt; '12' ">
								<xsl:value-of select="number(substring($datetimestring, 12,2))-12" />
							</xsl:when>
							<xsl:otherwise>
								<xsl:value-of select="substring($datetimestring, 12,2)" />
							</xsl:otherwise>
				...
				<xsl:if test="substring($datetimestring, 15,2)">
					:
					<xsl:value-of select="substring($datetimestring, 15,2)" />
					<xsl:if test="substring($datetimestring, 18,2) and $show-seconds='yes'">
						:
						<xsl:value-of select="substring($datetimestring, 18,2)" />
					</xsl:if>
				</xsl:if>
				<xsl:if test="not($display-as-24-hour)">
					<xsl:choose>
						<xsl:when test="substring($datetimestring, 12,2) &gt;= '12'">&nbsp;PM
						</xsl:when>
						<xsl:otherwise>&nbsp;AM
						</xsl:otherwise>
					</xsl:choose>
		...
		<xsl:if test="$show-timezone='yes'">
			<xsl:variable select="substring($datetimestring, 11)"
				name="tz-substring" />
			<xsl:if
				test="(contains($tz-substring, '+') or contains($tz-substring, '-')) and not(substring($tz-substring, string-length($tz-substring)-4) = '00:00') and not(substring($tz-substring, string-length($tz-substring)-2) = '+00') and not(substring($tz-substring, string-length($tz-substring)-2) = '-00')">
				<xsl:choose>
					<xsl:when test="contains($tz-substring, '+')">&nbsp;+
						<xsl:variable select="substring-after($tz-substring, '+')"
							name="tz-string" />
						<xsl:value-of select="substring($tz-string, 1, 2)" />
						<xsl:if test="string-length($tz-string) > 3">
							:
							<xsl:choose>
			...
			<table width="100%" cellpadding="1" cellspacing="0">
				<tr>
					...
					<td align="right" valign="bottom">
						<!-- healthcare facility -->
						<xsl:if test="v1:context/v1:health_care_facility">
							Facility:
							<xsl:value-of select="v1:context/v1:health_care_facility/v1:name" />
						</xsl:if>
					</td>
					<td rowspan="2" width="40" valign="top">
						<form id="embeddedShareForm" METHOD="POST"
							ENCTYPE="application/x-www-form-urlencoded" ACTION="/bm/test/">
							<INPUT TYPE="hidden" name="uri" id="bmTargetURI"
								value="replace-with-server-and-ehrID/{$compositionnode/../v1:uid/v1:value}" />
							<INPUT TYPE="hidden" name="title"
								value="{$compositionnode/v1:name/v1:value}, {$compositionnode/v1:context/v1:start_time/v1:value}, {$compositionnode/v1:context/v1:health_care_facility/v1:name}" />
							<INPUT TYPE="hidden" name="tags" value="" />
							<img class="embeddedShareButton" src="/static/images/Share.png"
								height="40" width="40" onclick="doMark" alt="Submit button" />
						</form>
					</td>
				</tr>
				<tr>
					<td align="right" valign="top" halign="right">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
						<xsl:choose>
							<xsl:when
								test="$compositionnode/v1:composer[@xsi:type='PARTY_IDENTIFIED']">
								Reported
								by:
								<xsl:value-of select="$compositionnode/v1:composer/v1:name" />
							</xsl:when>
							<xsl:when
								test="$compositionnode/v1:composer[@xsi:type='PARTY_RELATED']">
								Reported by:
								<xsl:value-of select="$compositionnode/v1:composer/v1:name" />
							</xsl:when>
							<xsl:when test="$compositionnode/v1:composer[@xsi:type='PARTY_SELF']">
								Reported by:
								subject
							</xsl:when>
						</xsl:choose>
				...
				<td align="right" valign="top">
					...
					<xsl:call-template name="generic-date">
						<xsl:with-param name="datetimestring"
							select="$contextnode/v1:start_time/v1:value" />
					</xsl:call-template>&nbsp;
	...
	<xsl:template name="generic-entry-header">
		...
		<!--<xsl:if test="$entrynode/protocol"><br/>Protocol<xsl:call-template 
			name="generic-ITEM_STRUCTURE"> <xsl:with-param name="itemsnode" select="$entrynode/protocol"/> 
	...
	<!-- .............. Start openEHR class templates ............. Every one 
		of these templates corresponds to a class in openEHR reference model (openEHR 
		v1.1 used). -->
	...
	<xsl:template name="generic-EVENT_CONTEXT">
		...
		<p style="margin:0px;">
			...
			<!-- start and end times -->
			<xsl:choose>
				<xsl:when test="$targetnode/v1:end_time/v1:value">
					Start time:
				...
				<xsl:otherwise>
					...
					<xsl:call-template name="generic-date">
						<xsl:with-param name="datetimestring"
							select="$targetnode/v1:start_time/v1:value" />
					</xsl:call-template>
					<br />
					Time:
					<xsl:call-template name="generic-time">
			...
			<xsl:if test="$targetnode/v1:health_care_facility">
				<br />
				Facility:
				<xsl:value-of select="$targetnode/v1:health_care_facility/v1:name" />
			</xsl:if>
	...
	<xsl:template name="generic-HISTORY">
		...
		<xsl:if test="$targetnode/v1:period">
			<br />
			<xsl:value-of select="$targetnode/v1:period/name/value" />
			:
		...
		<xsl:if test="$targetnode/v1:duration">
			<br />
			<xsl:value-of select="$targetnode/v1:duration/name/value" />
			:
			<xsl:call-template name="generic-DV_DURATION">
		...
		<xsl:for-each select="$targetnode/v1:events">
			<br />
			<xsl:call-template name="generic-EVENT">
	...
	<xsl:template name="generic-ITEM_STRUCTURE"
		xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
		<xsl:param name="itemsnode" />
		<xsl:param name="itemsnodeType">
           <!-- xsl:value-of select="local-name($dvnode/attribute::xsi:type)"/ -->
           <xsl:choose>
 			 <xsl:when test="contains($itemsnode/attribute::xsi:type, ':')">
 			   <!-- Remove prefix if the attribute value contains a colon sign. -->
			   <xsl:value-of select="substring-after($itemsnode/attribute::xsi:type, ':')"/>
			 </xsl:when>
			 <xsl:otherwise>
			   <xsl:value-of select="$itemsnode/attribute::xsi:type"/>
			 </xsl:otherwise>
			</xsl:choose>			           
        </xsl:param>
		<xsl:choose>
			<!-- known item tree -->
			<xsl:when test="$itemsnodeType='ITEM_TREE'">
			...
			<!-- known item list -->
			<xsl:when test="$itemsnodeType='ITEM_LIST'">
				<xsl:if test="$itemsnode/v1:items">
			...
			<!-- known item table -->
			<xsl:when test="$itemsnodeType='ITEM_TABLE'">
				<xsl:if test="$itemsnode/v1:items">
			...
			<xsl:when test="$itemsnodeType='ITEM_SINGLE'">
			...
			<xsl:otherwise>
				<span color="#f00;">
					---- Error displaying item structure: Could not detect concrete
					type (
					<xsl:value-of select="$itemsnode/attribute::xsi:type" />
					).
				</span>
	...
	<xsl:template name="generic-ITEM_LIST">
		<xsl:param name="itemsnode" />
		<xsl:for-each select="$itemsnode/v1:items">
			<!-- in theory should be able to call generic-ELEMENT directly but not 
				100% sure this will work, depending on XML instance -->
			<xsl:call-template name="generic-ELEMENT">
				<xsl:with-param name="elementnode" select="." />
			</xsl:call-template>
		</xsl:for-each>
	...
	<!-- Generic ITEM If we have child "items" not null of xsi:type equal to 
		cluster then calls generic cluster template, otherwise calls generic element 
		template -->
	<xsl:template name="generic-ITEM">
		<xsl:param name="itemnode" />
		<xsl:choose>
			<xsl:when test="$itemnode/v1:items or $itemnode[@xsi:type='CLUSTER']"> <!-- TODO: make it also work for xsi:type='PrefixX:... -->
				<xsl:call-template name="generic-CLUSTER">
	...
	<xsl:template name="generic-DV_BOOLEAN">
		...
		<xsl:choose>
			<xsl:when test="$boolnode/v1:value='true'">
				Yes
			</xsl:when>
			<xsl:otherwise>
				No
	...
	<xsl:template name="generic-DV_CODED_TEXT">
		...
		(<xsl:value-of
			select="$targetnode/v1:defining_code/v1:terminology_id/v1:value" />::<xsl:value-of select="$targetnode/v1:defining_code/v1:code_string" />)
	</xsl:template>
	...
	<!-- Generic DV_DATE_TIME (in format is '2006-05-05T10:44:46'; out format 
		is '10:44') uses generic-date; assumes XML instance will use standard date 
		"value" element uses generic-time; assumes XML instance will use standard 
		date "value" element -->
	...
	<!-- Generic DV_ORDERED -->
	<xsl:template name="generic-DV_ORDERED">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<span style="font-size:80%">
					UNSUPPORTED DV_ORDERED CHILD TYPE ENCOUNTERED:
					<xsl:value-of select="$orderednode/attribute::xsi:type" />
				</span>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="generic-DV_PROPORTION">
		...
		<xsl:choose>
			<!-- ratio -->
			<xsl:when test="$targetnode/v1:type = '0'">
				::
				<xsl:value-of select="$targetnode/v1:denominator" />
			</xsl:when>

			<!-- unitary -->
			<xsl:when test="$targetnode/v1:type='1'">
				per
				<xsl:value-of select="$targetnode/v1:denominator" />
			</xsl:when>
			...
			<xsl:when test="$targetnode/v1:type='3' or $targetnode/v1:type='4'">
				/
				<xsl:value-of select="$targetnode/v1:denominator" />
			</xsl:when>
		</xsl:choose>
	...
	<xsl:template name="generic-DV_QUANTITY">
		...
		<xsl:variable name="ranges-text">
			...
			<xsl:if test="$targetnode/v1:other_reference_ranges">
				<xsl:for-each select="$targetnode/v1:other_reference_ranges">
					<xsl:choose>
						<xsl:when
				...
				<xsl:choose>
					<xsl:when
						...
						<span style="color:#0a0;" title="{$ranges-text}">
							...
							<xsl:value-of select="$targetnode/v1:units" />
						</span>
					</xsl:when>
					<xsl:otherwise>
	...
	<!-- Generic DV_TEXT hyperlinks the text if hyperlink is present ignores 
		formatiing, mapping, language, encoding -->
	<xsl:template name="generic-DV_TEXT">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select="$textnode/v1:value" />
				<!-- <xsl:call-template name="replaceNL"> <xsl:with-param name="string" 
					select="$textnode/v1:value"/> </xsl:call-template> -->
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="summary-histology-HISTORY">
		...
		<xsl:for-each
			select="$targetnode/v1:events/v1:data/descendant-or-self::*/v1:items">
			<xsl:if test="@xsi:type='ELEMENT'">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
				<xsl:call-template name="generic-ELEMENT">
					<xsl:with-param name="elementnode" select="." />
				</xsl:call-template>
			</xsl:if>
	...
	<xsl:template name="alt-CLUSTER">
		...
		<xsl:for-each select="$clusternode/v1:items">
			...
			<xsl:call-template name="alt-ITEM">
				<xsl:with-param name="itemnode" select="." />
				<xsl:with-param name="displayboldname">
					no
				</xsl:with-param>
			</xsl:call-template>
		</xsl:for-each>
	...
	<xsl:template name="ocean-observation-HISTORY" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<xsl:for-each select="$targetnode/v1:events">
			<xsl:choose>
				<xsl:when
					test="not($targetnode/v1:events/v1:data/v1:item or $targetnode/v1:events/v1:data/v1:item[xsi:type='ITEM_SINGLE'])">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
					<xsl:if test="count(./parent::node()/events) > 1">
						<br />
						Specimen collected:
						<xsl:call-template name="generic-DV_DATE_TIME">
				...
				<xsl:otherwise>
					<br />
					<xsl:choose>
						<xsl:when test="v1:data/v1:item/v1:value[@xsi:type='DV_QUANTITY']">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
							<xsl:value-of select="v1:data/v1:item/v1:value/v1:magnitude" />&nbsp;
							<xsl:value-of select="v1:data/v1:item/v1:value/v1:units" />
						</xsl:when>
						<xsl:otherwise>
							<xsl:call-template name="generic-DATA_VALUE">
	...
	<xsl:template name="ocean-observation-event-ITEM_STRUCTURE"
		...
		<xsl:choose>
			<xsl:when
				test="$itemsnode/v1:item or $itemsnode/v1:item[@xsi:type='ITEM_SINGLE']">&nbsp;  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
				<b>
					<xsl:call-template name="generic-DATA_VALUE">
			...
			<xsl:when
				test="$itemsnode/v1:item or $itemsnode/v1:item[@xsi:type='ITEM_TABLE']">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
				<table cellpadding="1" cellspacing="0"
					style="font-size:12px;border-top:solid 1px#ddd;">
					<xsl:for-each select="$itemsnode/v1:items">
	...
	<xsl:template name="ocean-observation-ELEMENT" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
		<xsl:param name="elementnode" />
		<tr>
			<xsl:choose>
				...
				<xsl:when test="$elementnode/v1:value[@xsi:type='DV_QUANTITY']">
					<xsl:variable name="val"
						select="number($elementnode/v1:value/v1:magnitude)" />
					<xsl:variable name="ranges-text">
						<xsl:if test="$elementnode/v1:value/v1:normal_range">
							<xsl:variable name="norm-val-low"
								select="number($elementnode/v1:value/v1:normal_range/v1:lower/v1:magnitude)" />
							<xsl:variable name="norm-val-high"
								select="number($elementnode/v1:value/v1:normal_range/v1:upper/v1:magnitude)" />
							<xsl:choose>
								<xsl:when
									test="$val &gt;= $norm-val-low and $val &lt;= $norm-val-high">
									Value is within normal range
								</xsl:when>
								<xsl:otherwise>
									Value is outside normal range
					...
					<td align="left" valign="top" style="border-bottom:solid 1px #ddd;">
						<xsl:choose>
							<xsl:when
								...
								<span style="color:#0a0;" title="{$ranges-text}">
									<xsl:choose>
										<xsl:when test="contains($elementnode/v1:value/v1:magnitude, '.')">
											.
											<xsl:value-of
	...
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-COMPOSITION')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-OBSERVATION')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<xsl:if
			test="v1:data/v1:events/v1:data/v1:items or v1:data/v1:events/v1:data/v1:item or v1:data/v1:events/v1:data/v1:columns">
	...
	<!-- DEFAULT EVALUATION -->
	<xsl:template
		match="*[starts-with(@archetype_node_id,'openEHR-EHR-EVALUATION')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!-- DEFAULT INSTRUCTION -->
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-INSTRUCTION')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ACTION')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="action-ITEM_STRUCTURE" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">  <!-- TODO: make it also work for xsi:type='PrefixX:... -->
		<xsl:param name="itemsnode" />
		<xsl:choose>
			<!-- known item tree -->
	...
	<!-- DEFAULT ITEM_TREE -->
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ITEM_TREE')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<xsl:call-template name="generic-ITEM_TREE">
			<xsl:with-param name="itemsnode" select="." />
		</xsl:call-template>
	...
	<!-- DEFAULT ITEM_TABLE -->
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ITEM_TABLE')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<xsl:call-template name="generic-ITEM_TABLE">
			<xsl:with-param name="itemsnode" select="." />
		</xsl:call-template>
	...
	<!-- DEFAULT ITEM_SINGLE -->
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ITEM_SINGLE')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<xsl:call-template name="generic-ITEM_SINGLE">
			<xsl:with-param name="itemsnode" select="." />
		</xsl:call-template>
	...
	<!-- DEFAULT ITEM_LIST -->
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ITEM_LIST')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		<xsl:call-template name="generic-ITEM_LIST">
			<xsl:with-param name="itemsnode" select="." />
		</xsl:call-template>
	...
	<xsl:template
		match="*[starts-with(@archetype_node_id, 'openEHR-EHR-ADMIN_ENTRY')]"
		xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
...
<!-- ***** BEGIN LICENSE BLOCK *****
	...
	The Original Code is - EhrView openEHR RM generic html transform (base.xslt).
	
	The Initial Developer of the Original Code is - Ocean Informatics 
	Pty Ltd. Portions created by the Initial Developer are Copyright (C) 2006-2008 
	the Initial Developer. All Rights Reserved.
	
	Contributor(s): 
	  Lisa Thurston, Heath Frankel

    Several modifications have been added 2010-2012 by 
    the Department of Biomedical Engineering, Linköping University, Sweden
	Contributor(s): 
	  Daniel Karlsson, Erik Sundvall, Martin Eneling, Mikael Nyström
	  
	Alternatively, the contents of this file may be used under the terms of
	either the GNU General Public License Version 2 or later (the "GPL"), or
	the GNU Lesser General Public License Version2.1 or later (the "LGPL"),
