...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
  ...
  <xsl:template name="generic-ITEM_STRUCTURE" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$itemsnode[fn:ends-with(string(./attribute::xsi:type),'ITEM_TREE')]">
      
       fn:ends-with(string($x/attribute::xsi:type),'COMPOSITION')
  ...
  <xsl:template name="action-ITEM_STRUCTURE" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:choose>
      <!-- known item tree -->
      <xsl:when test="$itemsnode[ends-with(data(@xsi:type), 'ITEM_TREE')]">
        <xsl:if test="$itemsnode/oe:items">
      ...
      <!-- known item list -->
      <xsl:when test="$itemsnode[ends-with(data(@xsi:type), 'ITEM_LIST')]">
        <xsl:if test="$itemsnode/oe:items">
      ...
      <!-- known item table -->
      <xsl:when test="$itemsnode[ends-with(data(@xsi:type), 'ITEM_TABLE')]">
        <xsl:if test="$itemsnode/oe:items">
          <xsl:call-template name="generic-ITEM_TABLE">
            <xsl:with-param name="itemsnode" select="$itemsnode"/>
          </xsl:call-template>
